...
@Component
public class ReversalQueueListener {
    private static final Logger logger = LoggerFactory.getLogger(ReversalQueueListener.class);
    ...
    @Value("${app.queue.dlq.reversal.name}")
    ...
    public ReversalQueueListener(SqsClient sqs, ProcessTransactionService process) {
        this.sqs = sqs;
    ...
    void start() {
        ...
        worker = new Thread(this::loop, "sqs-reversal-poller");
        worker.setDaemon(true);
        worker.start();

        logger.info("SQS reversal poller iniciado. queue='{}' url='{}' waitSeconds={} maxMessages={} visibilityTimeout={}",
    ...
    void stop() {
        ...
        logger.info("Encerrando SQS reversal poller...");
        if (worker != null) {
        ...
        while (running.get()) {
            try {
                ...
                for (Message msg : messages) {
                    try {
                        logger.info("Mensagem recebida na fila sqs reversal {}", msg.body());
